...
int find_first_col(double **H,int v,int length)
{
	int col;
...
void makeH(double ***H,int id,double *names,int first,int last,int length)
{
	int c;
	for(c=first;c<last;c++){
		double v2=names[c-first+1];
		int v=(int)v2;
		int col=find_first_col(*H,v,length);
		(*H)[v][col]=id;
	}
...
int FC(double **B,int *pos,double *EP1,double *EP2,int length,int row,int column)
{
	...
	int NNZ=edge_matrix_double(B,row,column);
	if(n<=1){
		*pos=1;
		*EP1=1;
		*EP2=1;
...
void PL(double **W,double *W_names,double error,int *pos,double *EP1,double *EP2,double ***new_W,double **new_W_names,int length,int edge,int row,int column)
{
	...
	int sink=IX[n-1];
	double pi_pink=D[n-1]/sum_1_double(D,n)*1.0;
	...
	*new_W_names=init_matrix_double(n);
	for(r=0;r<n;r++){
		flection[IX2[r]]=r;
	...
	if(*new_W==NULL)
		(*new_W)=init_2_matrix_double(n,n);
...
void NCMA(double **W,double *W_names,double ***H,int *id,double err,double ***new_W,double **new_W_names,int edge,int row,int column)
{
	...
	if(EP1<0.5&&EP2<0.5){
		...
		(*id)++;
		if(pos>0)
			NCMA(W,W_names,H,id,err,new_W,new_W_names,edge,row,column);
		...
		(*id)++;
		if(n-pos>1)
			NCMA(W_B,W_B_names,H,id,err,&W_B,&W_B_names,edge_matrix_double(W_B,n-pos,n-pos),n-pos,n-pos);
		if(!new_W)
			(*new_W)=zeros_double(n,n);
...
void fec(double **adj,int row,int column,int edge)
{
	...
	double **H=init_2_matrix_double(n,n);
	int id=1;
	edge*=2;
	int c;
